...
### Incorporating New Gender Controlls
...
m = df_comp_stat %>%
  ...
  filter(dep_length > 6 & dep_length < 14) %>% #The dep lengths for the matrix condition
  mutate(test = if_else(test == "matrix", "hierarchy", "unboundedness")) %>%
  mutate(gap = if_else(gap == "nogap", "no-gap", gap)) %>%
  mutate(gap = if_else(gap == "gap", 1, -1),
         wh = if_else(wh == "what", 1, -1)) %>%
  lmer(surprisal ~ wh * gap * test + (wh + gap || item) + (wh + gap || dep_length), data = .)
summary(m)
...
ggplot(aes(x=island, y=m, ymin=lower, ymax=upper, fill=gap)) +
   theme_bw() +
    ...
    theme(legend.position="right", 
          ...
          axis.title.x=element_blank(),
          legend.title=element_blank())
ggsave("./images/cnp.pdf",height=3.3,width=10, device = cairo_pdf)
```
...
## Percentage Decrease in Island conditions
